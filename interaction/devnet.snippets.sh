...
WALLET_BOB="${PWD}/elrdon-escrow/wallets/bob.pem"
...
######## GET ESDT
 ...
 getEsdts() {
    ...
    --proxy=${PROXY} \
    --function="getEsdts" > ${PWD}/elrond-escrow/interaction/getEsdts.json

    ESDTS_HEX=$(erdpy data parse --file="${PWD}/elrond-escrow/interaction/getEsdts.json" --expression="data[1]['hex']")
    ESDTS=$(echo ${ESDTS_HEX} | xxd -r -p)
    echo "ESDTS: ${ESDTS}"
    echo -e "\nESDTS: ${ESDTS}" >> ${PWD}/elrond-escrow/interaction/getEsdts.json
}
...
FUNCTION="addOffer"
FUNCTION_HEX="$(echo -n ${FUNCTION} | xxd -p -u | tr -d '\n')"
...
addOffer() {
    ...
    --data="ESDTTransfer@${TOKEN_TO_HEX}@${AMOUNT_TO_HEX}@${FUNCTION_HEX}@${TOKEN_TO_HEX}@${AMOUNT_TO_HEX}@${TOKEN_FROM_HEX}@${AMOUNT_FROM_HEX}@${BOB_ADDRESS_HEX}"  
}
...
escrowReverse() {
    ...
    --gas-limit=10000000 \
    --receiver=${CONTRACT_ADDRESS} \
    --data="ESDTTransfer@${TOKEN_FROM_HEX}@${AMOUNT_FROM_HEX}@${FUNCTION_HEX}@${TOKEN_FROM_HEX}@${AMOUNT_FROM_HEX}@${TOKEN_TO_HEX}@${AMOUNT_TO_HEX}@${ALICE_ADDRESS_HEX}"  
}
